import { contextual, createContext, readContext, writeContext } from '../core';
...
export function getDOMContext(): DOMContext {
  const context = readContext(DOM_CONTEXT);
  if (context) {
    return context;
  }
  throw new Error(
    'Attempted to use a lifecycle hook outside of a custom element setup.',
  );
}
...
export function onDisconnected(
  callback: DOMContextMethods['disconnected'],
): void {
