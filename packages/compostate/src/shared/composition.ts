...
const COMPOSITION_CONTEXT = createContext<CompositionContext | undefined>(
  undefined,
);

export function createCompositionContext<T>(cb: () => T): T {
  return contextual(() => {
    writeContext(COMPOSITION_CONTEXT, {
      mounted: [],
      unmounted: [],
      effect: [],
      updated: [],
...
export function getCompositionContext(): CompositionContext {
  const context = readContext(COMPOSITION_CONTEXT);
  if (context) {
    return context;
  }
  throw new Error(
    'Attempted to use a lifecycle hook outside of a component setup.',
  );
}
...
export function onMounted(
  callback: CompositionContextMethods['mounted'],
): void {
  getCompositionContext().mounted.push(callback);
}

export function onUnmounted(
  callback: CompositionContextMethods['unmounted'],
): void {
  getCompositionContext().unmounted.push(callback);
}

export function onUpdated(
  callback: CompositionContextMethods['updated'],
): void {
